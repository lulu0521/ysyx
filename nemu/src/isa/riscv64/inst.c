...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc + 4; s->dnpc = (BITS(src1 + src2,31,1)<<1); j_flag = 1);
  ...
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(dest) = *csr(src2); *csr(src2) = src1 | *csr(src2) ); 
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , I, s->dnpc = cpu.mepc + 4; if(BITS(cpu.mstatus,7,7)) cpu.mstatus |= 0x8; else cpu.mstatus &=~0x8; cpu.mstatus |=0x80); 
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(R(17),s->pc);etrace_flag = 1); 
