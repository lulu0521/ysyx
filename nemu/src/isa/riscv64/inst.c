...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + dest, 2, src2); data_flag=1; data_addr=src1 + dest);
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + dest, 1, src2); data_flag=1; data_addr=src1 + dest);
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(dest) = ((__int128_t)(sword_t)src1 * (__int128_t)(sword_t)src2) >>64);
  ...
  INSTPAT("??????1 ????? ????? 101 ????? 01110 11", divuw  , R, R(dest) = (sword_t)(int32_t)((uint32_t)src1 / (uint32_t)src2));
  ...
  INSTPAT("?1????? ????? ????? 000 ????? 01110 11", subw   , R, R(dest) = (sword_t)(int32_t)(BITS(src1 - src2,31,0)));
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, R(dest) = SEXT(BITS(src1 << BITS(src2,4,0),31,0),32));
  INSTPAT("??????1 ????? ????? 000 ????? 01110 11", mulw   , R, R(dest) = (sword_t)(int32_t)BITS(src1*src2,31,0));
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(dest) = (sword_t)((int32_t)BITS(src1,31,0) / (int32_t)BITS(src2,31,0)));
  INSTPAT("?1????? ????? ????? 101 ????? 01110 11", sraw   , R, R(dest) = (sword_t)((int32_t)src1 >> BITS(src2,4,0)));
  INSTPAT("?0????? ????? ????? 101 ????? 01110 11", srlw   , R, R(dest) = (sword_t)((uint32_t)src1 >> BITS(src2,4,0)));
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw   , R, R(dest) = (int32_t)(src1 + src2));
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, R(dest) =  (sword_t)((int32_t)BITS(src1,31,0) % (int32_t)BITS(src2,31,0)));
