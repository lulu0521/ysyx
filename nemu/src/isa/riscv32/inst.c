...
static word_t immI(uint32_t i) { return BITS(i, 31, 20); }
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc + 4; s->dnpc = (BITS(src1 + src2,31,1)<<1));
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = (src1 < src2) ? 1:0);
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = ((sword_t)src1 >> BITS(src2,4,0)));
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + 4; s->dnpc = s->pc + (BITS(src1,31,1)<<1));

  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = (src1==src2) ? (s->pc + (BITS(dest,31,1)<<1)):(s->pc+4));
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, s->dnpc = (src1!=src2) ? (s->pc + (BITS(dest,31,1)<<1)):(s->pc+4));
