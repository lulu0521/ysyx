...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = Mr(src1 + src2, 4); data_flag=1; data_addr=src1 + src2);
  ...
  //INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(dest) = src2; ); 
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + 4; s->dnpc = s->pc + (BITS(src1,31,1)<<1); j_flag = 1);
